//
//  CULLogicalOrValue.cpp
//  cudalng
//
//  Created by Bernardo Breder on 20/05/15.
//  Copyright (c) 2015 Cuda. All rights reserved.
//

#include "CUL.h"

void CULLogicalOrValue::syntaxHead(CULContext *context) {
    CULBinaryValue::syntaxHead(context);
}

void CULLogicalOrValue::syntaxBody(CULContext *context) {
    CULBinaryValue::syntaxBody(context);
}

void CULLogicalOrValue::semantic(CULContext *context) {
    CULBinaryValue::semantic(context);
}
