//
//  CULShiftLeftValue.cpp
//  cudalng
//
//  Created by Bernardo Breder on 20/05/15.
//  Copyright (c) 2015 Cuda. All rights reserved.
//

#include "CUL.h"

void CULShiftLeftValue::syntaxHead(CULContext *context) {
    CULBinaryValue::syntaxHead(context);
}

void CULShiftLeftValue::syntaxBody(CULContext *context) {
    CULBinaryValue::syntaxBody(context);
}

void CULShiftLeftValue::semantic(CULContext *context) {
    CULBinaryValue::semantic(context);
}
