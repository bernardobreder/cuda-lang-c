...
unsigned char* stringbuilder_tostring(struct stringbuilder_t* self, unsigned int* length) {
    ...
    if (self->first && !self->first->next) {
        ...
        unsigned char* data = (unsigned char*) malloc(self->size * sizeof(unsigned char));
        if (!data) {
            return 0;
        }
        data[self->size] = 0;
    ...
    unsigned char* copy = (unsigned char*) malloc((self->size + 1) * sizeof(unsigned char));
    if (!copy) {
        free(data);
        return 0;
    }
    ...
    self->first->next = 0;
    strncpy((char*) copy, (char*) data, self->size);
    return copy;
}